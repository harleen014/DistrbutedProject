package com.distributed.userservice.config;
...
public class SwaggerConfiguration {
	...
	public Docket getDocket() {
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(getAPIInfo()).select()
				...
				.paths(regex("/userservice/api/v1.*")).build();

	}

	private ApiInfo getAPIInfo() {
		return new ApiInfoBuilder().title(environment.getProperty("news.swagger.user.authentication.title"))
				.description(environment.getProperty("news.swagger.user.authentication.description"))
