...
import com.distributed.articleservice.controller.ArticleController;
...
public class ArticleControllerTest {
	...
	public void test_whenAddArticle() throws ArticleAllreadyExistsException {
		...
		try {
			this.mockmvc.perform(post("/news/api/v1/article")
                    ...
                    .content(getJsonFromArticleModel(ArticleServiceUtil.convertArticleEntityToModel(this.entityList.get(0)))))
	...
	/*
	 * @Test public void test_whenfetchArticles() {
	 * when(articleService.getMyArticles(anyString())).thenReturn(this.entityList);
	 * when(environment.getProperty("newsapp.jwt.secret.key")).thenReturn("mc&ll$m")
	 * ; try { this.mockmvc.perform(get("/news/api/v1/articles")
	 * .header(HttpHeaders.AUTHORIZATION, generateToken(this.testUserEmail))
	 * .contentType(MediaType.APPLICATION_JSON)) .andExpect(status().isOk())
	 * .andDo(result ->{ List<ArticleModel> modelList =
	 * getArticleModelfromJsonList(result.getResponse().getContentAsString());
	 * assertThat(modelList.size()).isEqualTo(2); }); } catch(Exception ex) {
	 * ex.printStackTrace(); } }
	 * 
	 * @Test public void test_whenDeleteArticle() throws ArticleNotFoundException {
	 * this.entityList.remove(0);
	 * when(articleService.deleteArticle(anyString(),anyString())).thenReturn(this.
	 * entityList);
	 * when(environment.getProperty("newsapp.jwt.secret.key")).thenReturn("mc&ll$m")
	 * ; try { this.mockmvc.perform(delete("/news/api/v1/article")
	 * .header(HttpHeaders.AUTHORIZATION, generateToken(this.testUserEmail))
	 * .contentType(MediaType.APPLICATION_JSON) .param("author",
	 * this.entityList.get(0).getAuthor()) .param("publishedAt",
	 * this.entityList.get(0).getPublishedAt())) .andExpect(status().isOk())
	 * .andDo(result ->{ List<ArticleModel> modelList =
	 * getArticleModelfromJsonList(result.getResponse().getContentAsString());
	 * assertThat(modelList.size()).isEqualTo(1); }); } catch(Exception ex) {
	 * ex.printStackTrace(); } }
	 */
